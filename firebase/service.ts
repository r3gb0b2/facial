...
export const addAttendee = async (eventId: string, attendee: Omit<Attendee, 'id' | 'status' | 'eventId' | 'createdAt'>, supplierId?: string): Promise<void> => {
  ...
  if (supplierId) {
      attendeeData.supplierId = supplierId;
  }

  await newAttendeeRef.set(attendeeData);
};
...
export const addSupplier = (eventId: string, name: string, sectors: string[], registrationLimit: number): Promise<any> => {
    const suppliersRef = db.collection(EVENTS_COLLECTION).doc(eventId).collection(SUPPLIERS_COLLECTION);
...
export const updateSupplier = (eventId: string, supplierId: string, data: Partial<Supplier>): Promise<void> => {
    const supplierDoc = db.collection(EVENTS_COLLECTION).doc(eventId).collection(SUPPLIERS_COLLECTION).doc(supplierId);
    return supplierDoc.update(data);
};
...
export const getAttendeeCountForSupplier = async (eventId: string, supplierId: string): Promise<number> => {
    const attendeesRef = db.collection(EVENTS_COLLECTION).doc(eventId).collection(ATTENDEES_COLLECTION);
    const q = attendeesRef.where('supplierId', '==', supplierId);
...
export const registerAttendeeForSupplier = async (
    eventId: string,
    supplierId: string,
    attendee: Omit<Attendee, 'id' | 'status' | 'eventId' | 'createdAt' | 'supplierId'>
): Promise<void> => {
    const eventRef = db.collection(EVENTS_COLLECTION).doc(eventId);
    const supplierRef = eventRef.collection(SUPPLIERS_COLLECTION).doc(supplierId);
    const attendeesRef = eventRef.collection(ATTENDEES_COLLECTION);

    await db.runTransaction(async (transaction) => {
        // 1. Check if supplier exists and get their registration limit
        ...
        if (!supplier.active) {
            throw new Error("This registration link is currently disabled.");
        }

        // 2. Count current registrations for this supplier
        const attendeesQuery = attendeesRef.where('supplierId', '==', supplierId);
        const attendeesSnapshot = await transaction.get(attendeesQuery);
        const currentCount = attendeesSnapshot.size;

        // 3. Check if the limit has been reached
        if (currentCount >= supplier.registrationLimit) {
            throw new Error("Registration limit for this supplier has been reached.");
        }

        // 4. If all checks pass, register the new attendee
        const newAttendeeRef = attendeesRef.doc();
        let photoURL = attendee.photo;

        if (attendee.photo.startsWith('data:image')) {
            const photoStorageRef = storage.ref(`events/${eventId}/${newAttendeeRef.id}.png`);
            await photoStorageRef.putString(attendee.photo, 'data_url');
            photoURL = await photoStorageRef.getDownloadURL();
        }

        transaction.set(newAttendeeRef, {
            ...attendee,
            photo: photoURL,
            eventId: eventId,
