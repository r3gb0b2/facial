...
const CategoryModal: React.FC<SupplierCategoryModalProps> = ({ isOpen, onClose, onSave, categoryToEdit }) => {
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-700" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 bg-gray-900/50 rounded-b-2xl">
                    <button onClick={handleSave} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg">
                        {categoryToEdit ? t('sectors.modal.saveButton') : t('sectors.modal.createButton')}
                    </button>
                </div>
            </div>
        </div>
...
const SupplierCategoryManagementView: React.FC<SupplierCategoryManagementViewProps> = ({ categories, suppliers, onAddCategory, onUpdateCategory, onDeleteCategory, setError }) => {
    const { t } = useTranslation();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [categoryToEdit, setCategoryToEdit] = useState<SupplierCategory | null>(null);

    const handleOpenModal = (category: SupplierCategory | null = null) => {
    ...
    const handleDelete = async (category: SupplierCategory) => {
        const isCategoryInUse = suppliers.some(s => s.categoryId === category.id);
        if (isCategoryInUse) {
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-gray-700">
                ...
                {categories.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                        {categories.map((cat) => (
                            <li key={cat.id} className="bg-gray-900/70 p-4 rounded-lg flex items-center justify-between">
                                <p className="font-semibold text-white">{cat.name}</p>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => handleOpenModal(cat)} className="p-2 text-gray-400 hover:text-yellow-400 rounded-full hover:bg-gray-700">
                                        <PencilIcon className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => handleDelete(cat)} className="p-2 text-gray-400 hover:text-red-400 rounded-full hover:bg-gray-700">
                ...
                <div className="mt-8">
                    <button onClick={() => handleOpenModal()} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg">
