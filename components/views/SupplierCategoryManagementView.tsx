    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-700 flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 bg-gray-900/50 rounded-b-2xl">
          <button onClick={handleSave} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300">
...
const SupplierCategoryManagementView: React.FC<SupplierCategoryManagementViewProps> = ({
    ...
    const handleDelete = async (category: SupplierCategory) => {
        const isInUse = suppliers.some(s => s.categoryId === category.id);
        if (isInUse) {
