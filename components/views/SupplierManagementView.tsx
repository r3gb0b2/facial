...
const SupplierManagementView: React.FC<SupplierManagementViewProps> = (props) => {
    ...
    const { currentEventId, suppliers, attendees, sectors, categories, onAddSupplier, onUpdateSupplier, onDeleteSupplier, onSupplierStatusUpdate, setError } = props;
    ...
    const handleSaveEdit = async () => {
        ...
        if (!editingSupplier.name.trim() || !editingSupplier.categoryId) {
            setError(t('suppliers.noNameError'));
            return;
        }
    ...
    return (
        <div className="w-full max-w-4xl mx-auto space-y-10">
            {/* Generate Link by Category */}
            <div className="bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-gray-700">
                <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-3">
                    <LinkIcon className="w-7 h-7"/>
                    {t('suppliers.generateLinkTitle')}
                </h2>
                {categories.length > 0 ? (
                    <div className="flex flex-col md:flex-row gap-4">
                        <select 
                            ...
                            className="w-full bg-gray-900 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 flex-grow"
                        >
                        ...
                        <button 
                            ...
                            className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-3 px-4 rounded-lg flex items-center gap-2 justify-center w-full md:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                    ...
                    <p className="text-center text-gray-400">{t('suppliers.noCategoriesMessage')}</p>
                )}
            </div>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-gray-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                     <div>
                        <label htmlFor="category" className="block text-sm font-medium text-gray-300 mb-1">{t('admin.tabs.supplier_categories')}</label>
                        <select
                    ...
                    <button type="submit" disabled={isSubmitting || !selectedCategoryId} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg transition-colors disabled:bg-indigo-400 disabled:cursor-not-allowed">
                                ...
                                <div key={supplier.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                    <div className="flex-grow">
                                        <div className="flex items-center gap-3 mb-1">
                                            ...
                                            <span className="text-xs text-gray-400 bg-gray-700 px-2 py-0.5 rounded-full">{categoryMap.get(supplier.categoryId || '') || 'Sem Categoria'}</span>
                                        </div>
