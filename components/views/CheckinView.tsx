...
// FIX: Added XMarkIcon to the import list to resolve "Cannot find name 'XMarkIcon'" error.
import { SearchIcon, CheckCircleIcon, UsersIcon, ArrowDownTrayIcon, FaceSmileIcon, PencilIcon, TrashIcon, NoSymbolIcon, XMarkIcon } from '../icons.tsx';
...
const CheckinView: React.FC<CheckinViewProps> = ({ user, attendees, suppliers, sectors, currentEventId, currentEventName, onUpdateAttendeeDetails, onDeleteAttendee, onApproveSubstitution, onRejectSubstitution, onApproveSectorChange, onRejectSectorChange, onApproveNewRegistration, onRejectNewRegistration, setError, isVip = false }) => {
  ...
  const handleBulkStatusUpdate = async (status: CheckinStatus) => {
      try {
          ...
          // FIX: Explicitly provided generic type to new Set to avoid "unknown" to "string" assignment error.
  ...
  const filteredAttendees = useMemo(() => {
    ...
    return attendees.filter((attendee) => {
      ...
      // Filtro de idade
      if (attendee.age) {
          if (attendee.age < min || attendee.age > max) return false;
      } else if (ageMin || ageMax) {
          return false; // Oculta quem não tem idade se o filtro estiver ativo
      }
  ...
  const containerClass = "bg-neutral-900/50 backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-2xl border border-white/10";
  const inputClass = "w-full bg-black/50 border border-neutral-700 rounded-xl py-3 px-4 text-white focus:outline-none focus:ring-2 focus:ring-rose-500/50 transition-all text-sm";
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-32">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             ...
             <div className="flex gap-2">
                <input type="number" placeholder={t('checkin.filter.ageMin')} value={ageMin} onChange={(e) => handleFilterChange('ageMin', e.target.value)} className={inputClass} />
                <input type="number" placeholder={t('checkin.filter.ageMax')} value={ageMax} onChange={(e) => handleFilterChange('ageMax', e.target.value)} className={inputClass} />
             </div>
             <button onClick={handleExportToExcel} className="bg-white text-black font-black text-[10px] uppercase tracking-widest py-3 px-6 rounded-xl hover:bg-neutral-200 transition-all flex items-center justify-center gap-3">
      ...
      {selectedAttendee && (
        <AttendeeDetailModal
          ...
          onDelete={onUpdateDetails} onApproveSubstitution={onApproveSubstitution} onRejectSubstitution={onRejectSubstitution}
